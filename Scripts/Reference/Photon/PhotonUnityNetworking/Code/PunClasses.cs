        ...
        public GameObject Instantiate(string prefabId, Vector3 position, Quaternion rotation)
        {
            ...
            if (!cached)
            {
                ...
                if (assetBundle == null)
                {
                    ...
                    assetBundle = AssetBundle.LoadFromFile(prefabAB);
                    if (assetBundle == null)
                    {
                        Debug.Log("AB包:" + prefabAB + "不存在");
                    }
                }
                ...
                {
                    if (path.Split("/")[^1].Equals(prefabId + ".prefab"))
                    {
                        res = assetBundle.LoadAsset<GameObject>(path);
