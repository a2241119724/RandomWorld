...
namespace LAB2D
{
    public class GameTimeUI : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            globalLight = GameObject.FindGameObjectWithTag(ResourceConstant.GLOBAL_LIGHT_TAG_ROOT).GetComponent<Light2D>();
        }
        ...
        void Update()
        {
            ...
            if (_gameTime - gameTime == 1.0)
            {
                int hour = (int)gameTime / 3600;
                int minute = ((int)gameTime - hour * 3600) / 60;
                int second = (int)gameTime - hour * 3600 - minute * 60;
                time.text = string.Format("<color=green>游戏时间:</color>{0:D2}:{1:D2}:{2:D2}", hour, minute, second);
            }
        }
    }
}