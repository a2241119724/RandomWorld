namespace LAB2D
{
    ...
    public abstract class AGunObject : AWeaponObject
    {
        ...
        {
            GameObject g = ResourceManager.Instance.Instantiate(bulletName, this.gunHead.transform.position, Quaternion.identity);
            if (g == null)
            {
            ...
            g.GetComponent<ABulletObject>().Direction = this.gunHead.transform.position - this.transform.position;
            g.GetComponent<ABulletObject>().BulletSpeed = this.bulletSpeed;
            g.GetComponent<ABulletObject>().Damage = ((AWeapon)this.Item).GetDamage();
            g.GetComponent<ABulletObject>().Origin = this.player.GetComponent<Character>();
