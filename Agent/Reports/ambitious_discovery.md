...
| [DONE] | A010 | 装备掉落稀有度与对比强化系统（稀有度颜色+掉落光柱+对比弹窗+装备面板） | 成长与养成 / 战斗掉落 / UI与表现 | `AEquipment` 已有属性随机和 `BackpackItemQualityEnum` 9级品质、`CharacterData` 已有装备槽位和属性聚合，但缺少稀有度驱动的掉落视觉、装备对比 UI 和装备管理面板 | 创造"暗黑式"掉落兴奋感，每次击杀都有机会获得稀有装备，对比新旧装备后决定取舍，显著提升刷怪动力 | 激活已有装备数据层，为商店/打造/套装提供稀有度基础设施 | 中高 | 高 | P0 | GameplayAgent + UIAgent + ItemDataAgent | ScriptGenerateSkill + EditorToolSkill + CodeReviewSkill + TestSkill | Scripts/2D/Enum, Scripts/2D/Constant, Scripts/2D/Tool, Scripts/2D/Gameplay, Scripts/2D/UI, Scripts/2D/Editor, Scripts/2D/Character, GlobalInit, Game.unity, ResourcesLocal/Prefabs/UI | **已完成**。任务目录：`Agent/Reports/2026-05-10/ambitious_A010_Equipment_Loot/`；任务卡：`Agent/Reports/2026-05-10/ambitious_A010_Equipment_Loot/task_ambitious_A010_Equipment_Loot.md`；验证记录：`Agent/Reports/2026-05-10/ambitious_A010_Equipment_Loot/validation_ambitious_A010.md`；新增 `EquipmentRarityType.cs`、`EquipmentLootConstant.cs`、`EquipmentLootTool.cs`、`EquipmentLootManager.cs`、`EquipmentComparePopup.cs`、`EquipmentPanel.cs`、`EquipmentLootMenu.cs` 及 `.meta`；修改 `GlobalInit.cs`（+4行）、`CommonEnemyDeadState.cs`（+3行）、`SeekEnemyDeadState.cs`（+3行）。新增能力：6级装备稀有度（Common→Mythic）、敌人死亡按稀有度权重掉落装备、波次越高稀有概率越高、Legendary+装备随机极值属性、掉落位置浮动稀有度标签、装备对比弹窗（属性逐条对比+颜色标记提升/下降+替换/丢弃）、装备管理面板（F9切换+所有槽位+总属性加成）、Editor安装/卸载/测试菜单。UI 生成方式：运行时动态创建独立 Canvas（`Ambitious_A010_ComparePopup_Canvas` sortingOrder=250、`Ambitious_A010_EquipmentPanel_Canvas` sortingOrder=120）+ Editor 菜单辅助；未直接写入 `Game.unity`；未创建 `ResourcesLocal` Prefab。Tool 新增 `EquipmentLootTool.cs`（11个公共静态方法）、Enum 新增 `EquipmentRarityType.cs`（6值）、Constant 新增 `EquipmentLootConstant.cs`（8个分组）。静态验证通过；Unity 编译/Play Mode 待人工复验。剩余风险：装备拾取对比链路需在物品拾取系统中找到合适的挂接点；稀有度颜色特效需美术资源；数值平衡需 Play Mode 调优。 |
| [TODO] | A011 | BUFF/DEBUFF状态效果系统（持续伤害+减速+眩晕+增益图标+计时条） | 战斗体验升级 | A008 技能有基础 Buff（攻击提升/回血）、A004 波间奖励有临时 Buff，但缺少统一的状态效果管理、图标显示和堆叠机制 | 极大丰富战斗策略深度，让技能组合和敌人能力更有层次 | 为装备附加效果、药剂、陷阱和 Boss 机制提供状态框架 | 中 | 高 | P1 | GameplayAgent + AINPCAgent + UIAgent | ScriptGenerateSkill + CodeReviewSkill + TestSkill | Scripts/2D/Character, Scripts/2D/Gameplay, Scripts/2D/UI, Scripts/2D/Enum, Scripts/2D/Constant, Scripts/2D/Tool | 需修改 Character 战斗管线，风险中高但边界明确；UI 优先在 Game.unity 新增状态图标栏。 |
| [TODO] | A012 | 关卡星级评定与重玩激励系统（时间+击杀+收集→星级→奖励倍率→关卡选择） | 关卡与玩法 / 结算激励 | SessionResultData 已有评分和星级数据、WaveManager 已有波次记录，但缺少可见的星级评定 UI、关卡选择地图和重玩奖励 | 提供明确的关卡掌握度反馈和重玩动机，让玩家有"刷三星"目标 | 为关卡编辑器、无尽模式和排行榜提供评分基础设施 | 中 | 中 | P1 | GameplayAgent + LevelAgent + UIAgent | ScriptGenerateSkill + ConfigGenerateSkill + TestSkill | Scripts/2D/Gameplay, Scripts/2D/UI, Scripts/2D/Enum, Scripts/2D/Constant, Scripts/2D/Level, Game.unity, ResourcesLocal/Prefabs/UI | Medium risk, builds on existing session result data; UI 优先在 Game.unity 新增结算星级面板。 |
## 推荐优先开发

1. **A010 强烈推荐**：装备掉落稀有度与对比强化系统。项目已有完整装备数据层（`AEquipment`/`CharacterData.equipments`/`BackpackItemQualityEnum`/`EnemyDropManager`），但缺稀有度视觉、掉落反馈和装备对比UI。本候选能激活已有基础设施，创造"暗黑式"掉落兴奋感，中高风险但边界明确。
...
- **A010 [DONE] — 装备掉落稀有度与对比强化系统**
  ...
  - 验证记录：`Agent/Reports/2026-05-10/ambitious_A010_Equipment_Loot/validation_ambitious_A010.md`
  - 新增文件：`Scripts/2D/Enum/EquipmentRarityType.cs`、`Scripts/2D/Constant/EquipmentLootConstant.cs`、`Scripts/2D/Tool/EquipmentLootTool.cs`、`Scripts/2D/Gameplay/EquipmentLootManager.cs`、`Scripts/2D/UI/EquipmentComparePopup.cs`、`Scripts/2D/UI/EquipmentPanel.cs`、`Scripts/2D/Editor/EquipmentLootMenu.cs` 及 `.meta`
  - 修改文件：`Scripts/2D/GlobalInit.cs`（+4行）、`Scripts/2D/Character/Enemy/CommonEnemy/State/CommonEnemyDeadState.cs`（+3行）、`Scripts/2D/Character/Enemy/SeekEnemy/State/SeekEnemyDeadState.cs`（+3行）
  - 新增能力：6级装备稀有度（Common→Mythic）、敌人死亡按稀有度权重掉落装备、波次越高稀有概率越高、Legendary+装备随机极值属性、掉落位置浮动稀有度标签、装备对比弹窗（属性逐条对比+颜色标记提升/下降+替换/丢弃）、装备管理面板（F9切换+所有槽位+总属性加成）、Editor安装/卸载/测试菜单
  - UI 生成方式：运行时动态创建独立 Canvas（`Ambitious_A010_ComparePopup_Canvas` sortingOrder=250、`Ambitious_A010_EquipmentPanel_Canvas` sortingOrder=120）+ Editor 菜单辅助；未直接写入 `Game.unity`；未创建 `ResourcesLocal` Prefab
  - Tool：复用 `Tool.IsUIInputActive()`，新增 `EquipmentLootTool.cs`（11个公共静态方法）
  - Enum：新增 `EquipmentRarityType.cs`（6个枚举值）
  - Constant：新增 `EquipmentLootConstant.cs`（8个分组）
  - 验证结果：静态验证通过（无 UnityEditor 运行时引用、namespace 一致、Singleton/MonoBehaviour 模式一致）；`.meta` 由 Unity 生成；Unity 编译和 Play Mode 待人工复验
  - 剩余风险：装备拾取对比链路需在物品拾取系统中找到合适的挂接点；稀有度颜色特效需美术资源；数值平衡需 Play Mode 调优
