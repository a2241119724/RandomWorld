...
6. **几何可描述的素材直接程序化生成**：顶视火焰/光晕/同心环/放射状等"形状可公式描述"的素材，AI 文生图对立式形态先验极强（2026-09-03 顶视火焰 Z-Image-Turbo/Qwen-Image 三连败全是立式火舌），prompt 压不掉——直接 PIL 程序画（参照 `tmp/gen_flame_sprites.py`：极坐标半径场+帧间相位旋转，循环闭合、面积恒定可数值自检，顺带免费拿透明底）。
