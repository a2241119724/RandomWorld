...
- 2026-09 — `feat(map)`: M4 包4 地图兴趣点轮 2 上古洞府静态——`AncientCaveRuleService` 撒点约束（距中心≥20 格、间距≥30 格边界含）/`ShouldReveal` 走近 ≤8 格/`CaveState` 状态机（Hidden→Revealed→轮3 Exploring/Explored）/`RevealProgress` 淡入进度（60% 线性+尾部 ease-out）纯函数 + 6 单测；`AncientCaveManager` ITickable 0.5s 节流揭示扫描（单向门）+ 灵脉同款三路径撒点/存档 + `FindRevealedCaveNear` 查询（轮 3 交互入口）；`AncientCaveGlow` 程序化洞口暗斑+土金光环+微光尘（Hidden 无视觉、揭示淡入、读档跳过淡入直接全显）——「地图上有可发现物」，与危险区常显互补。轮 3 探索闭环（WorkerExploreTask/N 键读条/风险出怪/奖励结算）待做
