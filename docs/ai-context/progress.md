...
- 2026-09 — `feat(mind)`: EVT_GROUND_SLEEP 接活 + M2A 审查遗留全清零（3ee5b6a8）——核实中 6「四个修仙台词池死代码」已过时（突破/觉醒/工友嫉妒·敬仰四池均已被 CultivationManager/AwakenedPowerManager 接活）；顺藤发现 WorkerMindConstant 三键（BOUNTY_REFUSED/GROUND_SLEEP/FOUND_ITEM）有消费配置无生产者，接活最干净的 GROUND_SLEEP（WorkerSleepTask.Finish 无床分支 RecordEvent，自尊-2/一夜一次/无气泡），另两键留待节流设计（15 帧重试/拾取高频会刷爆记忆上限 24）；+1 Belief 防丢用例 1004/1004。**M2A 审查 2 高 5 中全部收口**
- 2026-09 — `fix(defence)`: M2A 中4 防守夜生存打断补派（90efdadd）——WorkerDefenceManager 加 ITickable 2s 节流扫描：防守任务被生存紧急打断（吃饭/睡觉/漫游）后 Worker 脱离生存任务即重新打分拉回防守（剩余时长），治参战者整夜人力流失；豁免与 CheckSurvivalEmergency 对齐（生存任务中/紧急中/Dead/Attack/Escape/对话暂停不补派，防派发-打断死循环），常态全员在防零分配短路；提取 DraftWorker 供入夜 draft 与补派共用。生存紧急阈值下沉 Domain（`IsSurvivalEmergency` 纯函数 + 4 常量，AWorker 五处字面量换常量单一来源零行为变化）；+6 单测 1003/1003。M2A 遗留仅剩中 6（台词池死代码）
...
- 2026-09 — `fix(runtime)`: 编辑器启动三连炸清零（ccdf84b6，用户贴堆栈点名）——① `Character.cs` 静态 `flashBlock = new MaterialPropertyBlock()` 在 prefab 反序列化触发 cctor 时抛 "CreateImpl is not allowed from MonoBehaviour constructor"（全部角色 prefab TypeInitializationException 刷屏），改惰性属性 `FlashBlock`（单测轮 icall 禁令的编辑器版本）；② `RoundCorner.SharedRoundMaterial` 的 `ResourceManager.Instance != null` 恒真（Singleton 懒创建语义），编辑模式 OnEnable 反向强拉 ResourceManager→LoadPrefabs 加载全部 prefab 连锁引爆①，改直接 `Shader.Find`；③ 并行会话包 4 漏 `SessionModifierManager` 的 ServiceLocator.Register（BuildInitializableList 强取 Get 在 GlobalInit.Awake 即 KeyNotFound），RegisterSafeServices 补一行。编译过 + 984/984；存档 `bug-fixes.md`
- 2026-09 — `fix(ui)`: RoundCorner 坏提交修复（4cd0a4d0，用户 Unity 编译报错点名）——6 处编译错误清零：三处 `IsActive` 补调用括号（UIBehaviour 是方法）；四处 `AddVert` 5 参改 `UIVertex` 结构体填充（本包 UGUI 1.0.0 无 (pos,color,uv0,uv1,uv2) 重载，uv 通道是 Vector4；`simpleVert` struct 拷贝起步，texcoord1=(w,h)/texcoord2.x=radiusPx 通道语义不变）；verify_tests.py 撤临时 stub 真件参编。真件编译过 + 984/984；圆角视觉/合批效果待用户回编辑器确认
- 2026-09 — `feat(cultivation)`: 包5 修仙深化轮 1 境界扩展+功法五行（cbca1c89）——`RealmLibrary` 六境（+元婴 3600 灵气/全维+15/HP400、化神 10800/封顶全维+25/HP800），金丹 QiToNext 0→3600 老档金丹自动可继续突破（RealmIndex 语义不变零迁移）；`GongFaLibrary` +玄水诀（水系防御内功练气档，功法池五行齐）；CultivationManager 巅峰 Tip 动态化（原硬编码金丹）；984/984。同轮发现：事件天气（e28746a9）与每局修饰符已被并行会话实施完毕——**包 4 三大子项（事件天气/修饰符/地图兴趣点）全部完成**，包 5 剩余：功法异能池扩充（异能走 SkillId 主动技能需技能系统配合）、炼丹炼器雏形、修炼-劳动经济调参（需 Play 实测）
- 2026-09 — `fix(tests)`: 环境性失败清零（c58cc97a）——47 败逐层归因后全灭，终态 **980 跑 / 980 过 / 0 败 / 6 跳**。分层根因与修法：① 构造期 icall 27 败——`WorkerData` 构造 Greed/Laziness 与 `AEquipment` 构造 `RankRandom`×8 都在构造函数里调 `UnityEngine.Random.Range`（icall，裸 Mono 必炸出 "assembly:<unknown assembly>"），换纯 C# 均匀随机（`WorkerPersonality.NextFloat` 转 internal 复用；`AEquipment.RandomFloatProvider` 默认换 `System.Random`，均匀分布语义等价、RankRandom 偏低值算法在随机源之外不受影响）；② 静态 ctor icall 4 败——`ASeek.s_wallLayerMask = LayerMask.GetMask(...)` 静态字段初始化炸 type initializer 连坐纯几何函数 `TryGetSlideDirection`，改惰性属性（运行时首次访问求值，行为无差异）；③ 测试缺 Ensure 16 败——LingGen/GongFa/GrowthData 测试在 `new GrowthData()` 后直接访问 null 集合字段（类契约「调用方先 Ensure」，与上轮 AwakenedPower 同款），17 处补 Ensure/先建集合；④ Wallet 初始 30 未对齐 2 败；⑤ ServiceLocator 裸环境缺 IGameTime 2 败——fixture 注册 FakeGameTime 桩。产品探针验证：LingGen/GongFa 全 API 干净进程全对，无产品回归
- 2026-09 — `fix(tests)`: 数值类失败定性清零（b5f83f50 + 8af26fe7）——上轮遗留 24 个数值期望失败逐例定性完毕（探针复现+实现在证）：**全部为测试自身缺陷或边界语义分歧，无产品回归**。12 处测试修正（LevelProgression 停在旧翻倍公式未跟 96d26170 线性曲线；Wave wave0 归一化/Boss 波参数误选；AwakenedPower 濒死 hp 误用 1 + GrowthData 构造不自动 Ensure；DayNight float 边界下溢 600f×0.7f=419.99999 + 采样 5 段越界；RealmRule 注释算错 speedMul；SessionResult 生存分模型与存活标志无关；WorkerRelationship 漏算记仇的 Affinity 惩罚；WaveSpawnPlan 误读 BeginNextWave 参数语义；PlayerDamagePolicy 无敌窗左闭右开；SpatialGrid 半径 200 排除对角远点）+ 1 处产品边界修复（WorkerTaskProgress 恰达 max 即完成，`>`→`>=`，实机 overshoot 无行为差异）。**TearDown 缺失是 TurnBattle 6 连败根因**——runner 补 TearDown 无条件执行后静态随机桩不再跨测试残留（UseSequence 桩恒 0.95+ → 必 miss → 零伤害；探针证实干净进程产品代码全对）。终态：**980 跑 / 933 过 / 47 败（全环境性：构造 engine 对象或依赖 ServiceLocator/Resources，裸 Mono 必炸）/ 6 跳**，非环境失败清零
- 2026-09 — `test(infra)`: 无人值守单测闭环（18581372）——`tmp/verify_tests.py` 一键管线：主程序集 ref.dll → Editor 测试程序集 → 内嵌极简 NUnit 反射 runner（Mono 4.7.1 BCL 编出 test_runner.exe）→ `mono.exe` 执行出统计。关键认知：Unity 定制 NUnit 是 net35（内部 `CallContext`），.NET 6 无 Remoting 跑不了，**必须 Unity 自带 Mono 运行时**。首跑 980 测试发现 3 个从未跑过的测试 bug 并修正（WorkerTaskQueue 断言 type 0 但 `t/100` 使任务全落 type 1/2 任何 runner 都不可能过；DangerZone/AncientCave 撒点候选距地图中心过近被中心约束遮蔽目标分支）。现状 980 跑/909 过/71 败/6 跳；71 败初分类=**47 环境性**（fixture 构造 Unity engine 对象/依赖 ServiceLocator 全局，裸 Mono 必炸，如 PersonalityDrift.NewWorker、ASeek 静态 ctor、GongFaLibrary 资产加载）+ **24 待定性**（数值期望类：LevelProgression×4、DayNight 边界×2、AwakenedPower×3 等）。附带发现：`Scripts/2D/RoundCorner.cs`（dc5a5552 圆角合批重写）**从未通过任何编译**——`IsActive` 无括号（UIBehaviour 是方法）+ `AddVert` 5 参重载不存在，Unity 下次刷新必编译红，verify_tests.py 暂以 stub 绕过待原作者修复
- 2026-09 — `fix(defend)`: M2A 审查中 7 驻守墙钟推进（4d5f8d37）——`AWorkerTask.IgnoresProgressMultiplier` 虚属性（默认 false 零行为变化），基类 Execute 按此豁免乘数链；DefendTask 覆写 true：驻守到黎明与个人效率无关，勤奋者不再提前收工（接敌帧暂停补时语义保留）；探索任务保持吃乘数（设计特性）。同日核实中 5「Ambition 采集未用」为审查误报（WorkerDefenceManager:131 自初版即采集，Domain 消费链完整）；spec/方案文档同步 + 17 个新脚本 .cs.meta 补齐（055e36cf，guid 防撞手写）
- 2026-09 — `feat(cave)`: M4 包4 地图兴趣点轮 3 探索闭环（收官）——`CaveExploreRuleService` 风险/奖励 roll 纯函数（妖兽35%/塌方10%/平安55%；功法40%/物资25%/装备25%/双倍10%）+4 单测；`WorkerTaskType.Explore`+`WorkerExploreTask`（DefendTask 同构：驻留 60s 吃 ProgressMultiplierProvider 全乘数链，Finish→`CompleteExplore` 结算、`GiveUpTask` 回滚 Exploring 占用，WorkerSpecific 指派者专属）；`AncientCaveManager` 探索状态机——N 键 30s 读条（移动/受击打断，打坐同款 `MeditateBreakMoveDistance`+`CharacterDamagedEvent`）、O 键派最近空闲 Worker（`SetTask` PushAssignment）、`CompleteExplore` 风险出怪（`EnemyManager.Create`×2~3 只洞府旁 3 格 Common）/塌方（`ReduceHp(maxHp×15%, null)`）/平安 + 奖励落地（`GongFaLibrary.All` 随机未学 `Learn` 洗牌重试/`ForceDrop` 包裹 `TryDropLoot`·`TryDropEquipment` 第 5 波档）、读档 Exploring 归一化 Revealed（任务不存档）；`CaveExploreHUD` 提示条（BattlePromptHUD 同款纯代码模式，进度显示）+ `GlobalInputProcessor.ProcessCaveExplore` N/O 分发（面板打开守卫同 JoinBattle）。方案三轮切片全量完成（`docs/open-issues/m4-p4-map-poi-plan.md` 可归档）
- 2026-09 — `feat(map)`: M4 包4 地图兴趣点轮 2 上古洞府静态——`AncientCaveRuleService` 撒点约束（距中心≥20 格、间距≥30 格边界含）/`ShouldReveal` 走近 ≤8 格/`CaveState` 状态机（Hidden→Revealed→轮3 Exploring/Explored）/`RevealProgress` 淡入进度（60% 线性+尾部 ease-out）纯函数 + 6 单测；`AncientCaveManager` ITickable 0.5s 节流揭示扫描（单向门）+ 灵脉同款三路径撒点/存档 + `FindRevealedCaveNear` 查询（轮 3 交互入口）；`AncientCaveGlow` 程序化洞口暗斑+土金光环+微光尘（Hidden 无视觉、揭示淡入、读档跳过淡入直接全显；轮 3 加 `MarkExplored` 枯竭灰暗）——「地图上有可发现物」，与危险区常显互补
